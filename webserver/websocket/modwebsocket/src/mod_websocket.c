...
handler_t handle_backend(server *srv, void *ctx, int revents) {
    ...
    if (revents & FDEVENT_NVAL || revents & FDEVENT_HUP || revents & FDEVENT_ERR) {
        ...
        if (siz == 0) {
            ...
            DEBUG_LOG(MOD_WEBSOCKET_LOG_DEBUG, "sdsxss",
                      "recv data from backend ( fd =", hctx->fd, "), size =", siz," data=",readbuf);
...
# undef	N
    ...
    if (buffer_is_empty(connection_hdr_value) ||
        ...
        strstr(change_toupper(connection_hdr_value->ptr), "UPGRADE") == NULL ||
        strstr(change_toupper(upgrade_hdr_value->ptr), "WEBSOCKET") == NULL) {
