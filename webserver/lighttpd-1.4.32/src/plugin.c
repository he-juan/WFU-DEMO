...
int valid_connection(connection *con)
{
	char *before;
	char *after;
	char tmp[80] = "";
	time_t now;
       ...
       size_t i;
	    ...
	    time(&now);          
            ...
            if ( before = strstr(ds->value->ptr, "phonecookie=\"") ) {
            	before += sizeof("phonecookie=\"") -1;
            	//printf("before is %s\n", before);
            	if ( after = strstr(before, "\"")) {

                	//printf("75. usercookie is %s, sessiontimeout is %ld, the req cookie is %s, now is %ld\n", 
                	//	usercookie, sessiontimeout, before, now);
                	//printf("tiem is %d", now);
                	strncpy(tmp, before, (after - before));
                	//printf("tmp is %s\n", tmp);
                	//printf("cookie is equal? %d\n", strcmp(usercookie, tmp));
                	//printf("time is over? %d\n", now < sessiontimeout);
                	if ((!strcmp(usercookie, tmp)) && (now < sessiontimeout)) {
                                sessiontimeout = now + WEB_TIMEOUT;
				strcpy (curuser, usertype);
                		valid = 1;
                            return valid;
                	}
               }
            }
            else  if ( before = strstr(ds->value->ptr, "phonecookie=") ) {
            	before += sizeof("phonecookie=") -1;
            	//printf("before is %s\n", before);
            	if ( after = strstr(before, ";")) {
            ...
            } else if ( before = strstr(ds->value->ptr, "gmicookie=\"")) {
		before += sizeof("gmicookie=\"") -1;
		if ( after = strstr(before, "\"")) {
			strncpy(tmp, before, (after - before));
			if((!strcmp(gmicookie, tmp)) && (now < gmisessiontimeout)) {
				gmisessiontimeout = now + WEB_TIMEOUT;
				strcpy (curuser, gmiusertype);
				valid = 1;
				return valid;
			}
		}
