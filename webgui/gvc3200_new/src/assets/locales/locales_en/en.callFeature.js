...
export default {
  ...
  call_037_tip: 'Set the video display mode to "Original proportion" or "Proportional add black edge". If set to "Original proportion", the device displays video in its original proportion that received from remote party, if the remote video display proportion is different from the device, the device will stretch or compress video to display it; If set to "Proportional add black edge", the device will display video in its original proportion, if still exists spare space, the device will add black edge on it. The default setting is "Original proportion".',
