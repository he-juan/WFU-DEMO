...
export default {
  ...
  acct_010: '语音信箱接入号',
  acct_010_tip: '这个号码通常是语音信箱入口接入码。例如：Asterisk服务器的语音信箱接入号为8500。',
  ...
  acct_023_tip_2: '该项设置设备监听和传输的本地SIP端口。默认值为5070。',
  ...
  acct_060: '语音编码',
  acct_060_tip: '设备支持多种不同的语音编码类型。设置优先选择列表中的语音编码，该列表与SDP信息的优先选择顺序相同。',
  ...
  acct_063: '语音帧/TX',
  acct_063_tip: '此项用于设置单包发送的语音帧的数量（建议基于以太网数据包的IS限制最大值为1500个字节（或120Kbit/s）） <br />设置该值时，要注意请求数据报时间（ptime，在SDP数据报中）是配置该参数的结果。该参数与上面编码性能表中的首编码有关或者在通话双方中协商实际应用的有效荷载类型。<br />若“语音帧/TX”设置超过最大允许值，设备将对应首编码的选择来使用并保存最大允许值。<br />推荐使用提供的默认设置，不正确的设置会影响语音质量。',
  ...
  acct_073_tip: '此项设置音频FEC负载类型。输入音频FEC有效荷载值范围为：96-126。默认为121。',

  acct_074: '音频RED有效荷载类型',
  acct_074_tip: '此项设置音频RED负载类型。输入音频RED有效荷载值范围为：96-126。默认为124。',
  ...
  acct_078: 'FEC有效荷载类型',
  acct_078_tip: '此项设置FEC负载类型。输入FEC有效荷载值范围为：96-126。默认为120。',

  acct_080: '开启FECC',
  acct_080_tip: '若勾选该项，在通话中可以控制对方的摄像头。但需对方支持FECC且允许远端控制本地摄像头，默认为勾选。',

  acct_081: 'FECC H.224有效荷载类型',
  acct_081_tip: '此项设置FECC H.224有效荷载类型。输入FECC H.224有效荷载类型值范围为：96-126。默认为125。',
  ...
  acct_090_tip_2: '视频封包模式0,1或自动；默认为0。',
  ...
  acct_091_zoom_tip: '设置H.264 Profile类型，可设置为基本档次或者高级档次。低级别的Profile类型更易解码，但是更高级别的Profile类型压缩率更高。通常，选择“高级档次”以获得高的视频压缩率。对于低CPU的设备，选择“基本档次”进行视频播放。',
  ...
  acct_093_tip: '输入H.265有效荷载值，范围为：96-126,默认为114。',

  acct_094: '启用演示',
  acct_094_tip: '如果设置为“是”，在视频通话和视频会议中设备将接收演示流。默认设置为“是”。',
  ...
  acct_108_tip: '若选择为“是”且服务器支持这项功能，则设备会检查“Refer-To”报头来处理呼叫。默认值为“否”。',
  ...
  acct_118_tip: '点击“浏览”按钮上传PC端音频文件。MOH音频文件必须是.wav或者.mp3格式。<br />注意：上传、处理MP3格式文件时可能会花费较长时间，请耐心等待。',
  ...
  acct_149: '自定义铃音',
  acct_149_tip: '该项指定对应左边的匹配来电的铃声。用户可以选择不同的铃声。',
  ...
  acct_153: '语音信箱接入号',
  acct_153_tip: '这个号码通常是语音信箱入口接入码。例如：Asterisk服务器的语音信箱接入号为8500。可以通过语音邮件应用进入语音邮箱。',

  acct_154: 'H.264视频大小',
  acct_154_tip: '支持的视频大小。默认为1080p.',

  acct_155: '视频比特率',
  acct_155_tip: '配置视频电话的比特率，可以根据网络环境调整的视频比特率。如果带宽允许的情况下建议增加比特率大小；如果带宽不够，视频质量将降低。默认值跟H.264 视频大小有关：<br />H.264 视频大小设置为1080p，设置值为“1Mbps~4Mbps”之间的整数值。<br />H.264 视频大小设置为720p，设置值为“512kbps~2Mbps”之间的整数值。<br />H.264 视频大小设置为4SIF/4CIF/VGA，设置值为“384kbps~1Mbps”之间的整数值。'
