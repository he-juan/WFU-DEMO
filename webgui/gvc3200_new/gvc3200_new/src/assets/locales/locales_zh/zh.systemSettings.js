...
export default {
  ...
  sys_sec_022: '导入用户证书',
  sys_sec_022_tip: '点击“上传”按钮上传用户证书文件。证书文件必须是.crt, .cer, .pem, .p12, 或者.pfx格式。',
  ...
  sys_per_005: '初始化位置',
  sys_per_005_tip: '设置终端启动时内置摄像机的初始位置。 “默认”表示终端启动时, 摄像机位置自动恢复到初始化位置。 “预置位置1”表示终端启动时, 摄像机位置恢复到预置位1, 但是预置位1必须已经设置过。 “关机时的位置” 表示终端启动时, 摄像机位置恢复到终端关机时的位置, 默认设置为“默认”。',

  sys_per_006: 'HDMI声音输出控制',
  sys_per_006_tip: '此项用于设置HDMI声音输出。当只连接HDMI 1时，结合“音频设备”配置，支持声音从HDMI 1输出。<br />注：HDMI声音输出控制当且仅当“音频设备”设置声音输出在HDMI时才生效。',
