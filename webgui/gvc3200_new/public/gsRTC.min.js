...
*
* */
function removeLevel(sdp){
    var lines = sdp.split("\n")
    for(var i = 0; i<lines.length; i++){
        if (lines[i].indexOf('level-asymmetry-allowed') >= 0 ) {
            console.info('remove level-asymmetry-allowed')
            lines.splice(i, 1)
            i--
        }
    }
...
function call(wsAddr, callback) {
    ...
    let sipCallInfo = {
        initialResolution: getScreenResolution(),   // 初始Invite视频协商分辨率,
...
function pausePresent(isMuteStream, callback){
    console.warn("isMuteStream:",isMuteStream)
    if (!gsRTC) {
        log.warn('gsRTC is not initialized')
        return
    }
...
GsRTC.prototype.shareScreen = function(data) {
    ...
    function getMediaCallBack(event){
        if(event.stream){
            ...
            stream.oninactive= function () {
                ...
                // gsRTC.on('stopShareScreen', stopCallback)
                window.isPresentShare = 'stop'
    ...
    this.off('shareScreen', data.callback)
}
...
GsRTC.prototype.switchScreenSource = function(data) {
    ...
    this.device.getMedia(gumData, data.constraints)
    this.off('switchScreenSource', data.callback)
}
...
GsRTC.prototype.stopShareScreen = function(data) {
    ...
    if(stream){
        ...
        this.RTCSession.setStream(null, type, true)
        this.RTCSession.doOffer(pc)
        this.off('stopShareScreen', data.callback)
    }
}
...
GsRTC.prototype.pauseScreenSource = function(data) {
    ...
    if (stream) {
        ...
        this.on('pauseScreenSource', data.callback)
        this.sendInviteQueue.push({action: 'pauseScreenSource', sdp: null})
        log.info('pause previous stream')
        this.RTCSession.processPauseStream(stream, pc, type, data.callback)
        this.off('pauseScreenSource', data.callback)
    }
}
...
PeerConnection.prototype.createMultiStreamRTCSession = function(conf){
    ...
    try {
        ...
        if(RTCPeerConnection.prototype.addTransceiver){
            ...
            pc.addTransceiver('video')
            // console.warn("Transceivers： ", pc.getTransceivers())
        }else {
...
PeerConnection.prototype.setRemoteDescriptionSuccess = function (pc) {
    ...
    // console.warn("window.isPresentShare: ", window.isPresentShare)
    // console.warn("window.isMainShare: ", window.isMainShare)
...
PeerConnection.prototype.getDecorateSdp = function () {
    ...
    if(This.gsRTC.enableMultiStream === true){
        ...
        // let newLevel
        //
        // var audioSdp = sdp.slice(0, positions[0]-1)
        // let mainSdp = sdp.slice(positions[0],positions[1]-1)
        // let slidesSdp = sdp.slice(positions[1], sdp.length-1)
        //
        ...
        //         if (lines[i].indexOf("level-asymmetry-allowed") >= 0) {
        //             newLevel = arr[1].slice(26)
        //             newArrs = arr[0] + " " + newLevel
        //         }
        //     }
        // }
        //
        // slidesSdp = removeLevel(slidesSdp)
        // // slidesSdp = slidesSdp.split('\n')
        // newSlides.push(slidesSdp)
        // newSlides.push(newArrs)
        // newSlides = newSlides.join("\n")
        //
        // mainSdp = removeLevel(mainSdp)
        // newMain.push(mainSdp)
        // newMain.push(newArrs)
        // newMain = newMain.join("\n")
        //
        // sdp = sdp.slice(0, positions[0]-1) +"\r\n"+ newMain+"\r\n"+ newSlides
        // // sdp = audioSdp + mainSdp + slidesSdp
        // console.warn("totalSdp:",sdp.toString())
...
PeerConnection.prototype.processPauseStream = function (stream, pc, type, callback) {
    if(stream != null && stream != undefined){
        log.info("MuteStream: stream id = " + stream.id);
    }
   if( (type == 'video' || type == 'slides') && stream.getVideoTracks().length > 0 ){
        for ( var i = 0; i < stream.getVideoTracks().length; i++ ) {
            log.warn("GsRTC.muteStream:",GsRTC.muteStream)
            if (GsRTC.muteStream){
                if ( stream.getVideoTracks()[i].enabled == true ) {
                    log.info("MuteStream exec mute video/slides");
                    stream.getVideoTracks()[i].enabled = false;
                }
            }
            else{
                if ( stream.getVideoTracks()[i].enabled == false ) {
                    log.info("MuteStream exec unmute video/slides");
                     stream.getVideoTracks()[i].enabled = true;
                }
            }
        }
    }
...
GsRTC.prototype.decorateMultiStreamSdp  = function(subSDP) {
    ...
    for(let i = 0; i<lines.length; i++){
        ...
        if(lines[i].indexOf('a=mid:1') >= 0){
            ...
            // "a=msid:af172ecd-1e37-4740-86ca-0cc69ceaef14_2"
            i++
        }
...
WebSocketInstance.prototype.createWebSocket = function(url, protocols){
    ...
    ws.onmessage = function (event) {
        ...
        if (isJsonString(event.data)) {
            ...
            Object.keys(parseDate).forEach(function(index) {
                console.log(index, parseDate[index])
                let code = parseDate[index].errorInfo.errorId
                if(code === 200){
                    if(parseDate[index].sdp) {
                        sdp = parseDate[index].sdp.data
                        if (index === 'updateMediaSessionRet') {
                            if (window.isPresentShare) {
                                ...
                                let maxBitRate = 2048
                                slidesSdp = setMaxBitrate(slidesSdp, 'video', maxBitRate)
                                slidesSdp = setXgoogleBitrate(slidesSdp, maxBitRate)
                                slidesSdp = removeREMBField(slidesSdp)
