...
function call(wsAddr, callback) {
    ...
    if( !gsRTC.isWFUShareScreenSupport()) {
        ...
        log.warn("支持屏幕共享的浏览器版本分别是：Chrome是72版本以上、opera是60版本以上、firefox是60版本以上、edge是79版本以上")
...
GsRTC.prototype.shareScreen = function(data) {
    ...
    if(gsRTC.sharingPermission === 3){
        gsRTC.sharingPermission = 3
        let seconds = Math.round(((new Date()).getTime() - gsRTC.openSharingTimeoutstartTime)/1000)
        ...
        if(seconds <= 25){
            ...
            data.stream.oninactive = null;
            let tracks = data.stream.getTracks();
            for (let track in tracks) {
                tracks[track].stop();
            }
            This.sokect.sendMessage({type: This.SIGNAL_EVENT_TYPE.PRESENT_RET, ctrlPresentationRet: This.CODE_TYPE.SHARE_SCREEN_TIMEOUT, reqId: This.reqId})
            data.callback( gsRTC.CODE_TYPE.SHARE_SCREEN_TIMEOUT)
        }
        ...
        case gsRTC.SIGNAL_EVENT_TYPE.PRESENT_RET.name:
            if(gsRTC.isNxx(2, code)){
                ...
                if(code === 408){
                    ...
                    gsRTC.trigger(gsRTC.action, {
                        codeType: code,
                        message: gsRTC.CODE_TYPE.SHARE_SCREEN_TIMEOUT
                    });
                    gsRTC.openSharingTimeoutstartTime = null
                }else if(code === 403){
                    let stream = gsRTC.MEDIA_STREAMS.LOCAL_PRESENT_STREAM
