...
PeerConnection.prototype.setRemoteDescriptionSuccess = function () {
    ...
    if(This.gsRTC.sendCtrlPresentation){
        log.info('current sharing permission: ' + gsRTC.sharingPermission)
        if(gsRTC.sharingPermission === 3 || gsRTC.sharingPermission === 4){
            This.gsRTC.sokect.sendMessage({type: gsRTC.SIGNAL_EVENT_TYPE.PRESENT_RET, ctrlPresentationRet: gsRTC.CODE_TYPE.SUCCESS, reqId: gsRTC.reqId})
        }else {
            This.gsRTC.sokect.sendMessage({type: gsRTC.SIGNAL_EVENT_TYPE.PRESENT, ctrlPresentation: { value: gsRTC.sharingPermission, reqId: gsRTC.reqId}})
        }
    }else {
