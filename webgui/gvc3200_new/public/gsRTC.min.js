    ...
    MediaDevice.prototype.getMedia = async function (data, constraints) {
        ...
        function onGetStreamFailed(error) {
            ...
            if(error.name === "OverconstrainedError" || error.name === "ConstraintNotSatisfiedError"){
                ...
                data.callback({error: error.name})
            }
        }
    ...
    MediaDevice.prototype.getScreenShareConstraints = function (data) {
        ...
        if (!!navigator.mozGetUserMedia) {
            screenConstraints = {
                audio: false,
                video: {
                    mozMediaSource: data.source || 'window',
                    mediaSource: data.source || 'window',
         ...
         * for Edge
         */
        if (gsRTC.getBrowserDetail().browser === "edge") {
            if (gsRTC.getBrowserDetail().version >= 17134 && !!navigator.getDisplayMedia) {
                screenConstraints = {
                    audio: false,
                    video: {
...
function call(wsAddr, callback) {
    ...
    if( (gsRTC.getBrowserDetail().browser === 'chrome'  && gsRTC.getBrowserDetail().version < 72   ) || (gsRTC.getBrowserDetail().browser === 'opera'  && gsRTC.getBrowserDetail().version < 60 ) || (gsRTC.getBrowserDetail().browser === 'firefox'  && gsRTC.getBrowserDetail().version < 60 )) {
        log.warn("当前浏览器版本不支持屏幕共享")
        log.warn("支持屏幕共享的浏览器版本分别是：Chrome是72版本以上、opera是60版本以上、firefox是60版本以上")
...
function beginScreen(callback){
    ...
    function getMediaCallBack(event){
        if(event.stream){
            try {
                ...
                if(gsRTC.serverAction === 'shareScreenRequest'){
    ...
    let constraints = {
        audio: false,
        video: {
            width: {ideal: gsRTC.initialResolution ? gsRTC.initialResolution.width ? gsRTC.initialResolution.width : 1920 : 1920},
            height: {ideal: gsRTC.initialResolution ? gsRTC.initialResolution.height ? gsRTC.initialResolution.height : 1080 : 1080},
            frameRate: {ideal: gsRTC.initialResolution ? gsRTC.initialResolution.framerate ? gsRTC.initialResolution.framerate : 15 : 15}
        }
    };
...
function stopScreen(callback){
    ...
    if(gsRTC.serverAction === 'stopShareScreenRequest'){
...
=======
>>>>>>> 02b2a30f2b6bf1e1a0849d0866dccf4020e15b2e
    sdp = This.modifyProfile(sdp)
    This.setRemote(sdp)
...
<<<<<<< HEAD
                ...
                let str = fmtpItem.config.substr(index, 19)
                let middle = fmtpItem.config.substr(19,2)
                let strs = middle.replace(/[\s\S]*/i,'e0')
                let pro = fmtpItem.config.substr(21, 2)
                let end = fmtpItem.config.substr(23)
                fmtpItem.config = str + strs +pro + end
=======
                let index = fmtpItem.config.indexOf('profile-level-id=')
...
GsRTC.prototype.CODE_TYPE = {
    ...
    CANCEL_PRESENT_ON:{codeType:104,message:'NotAllowedError'},
...
<<<<<<< HEAD
    // window.transceiverTarget = transceiverTarget
=======
>>>>>>> 02b2a30f2b6bf1e1a0849d0866dccf4020e15b2e
...
<<<<<<< HEAD
        if(browserDetail.browser === 'safari' && (browserDetail.UIVersion === "11.0.2" || browserDetail.UIVersion === "11.1.2" ) && transceiverTarget.sender){
            transceiverTarget.replaceTrack(stream.getTracks()[0])
=======
...
PeerConnection.prototype.processRemoveStream = function (stream, pc, type) {
    ...
    if(This.gsRTC.isReplaceTrackSupport() && pc.getTransceivers().length > 0){
        ...
        let browserDetail = This.gsRTC.getBrowserDetail()
<<<<<<< HEAD
        if(browserDetail.browser === 'safari' && (browserDetail.UIVersion === "11.0.2" || browserDetail.UIVersion === "11.1.2" ) && transceiverTarget.sender > 0){
=======
        if(browserDetail.browser === 'safari' && (browserDetail.UIVersion === "11.0.2" || browserDetail.UIVersion === "11.1.2" ) && transceiverTarget){
>>>>>>> 02b2a30f2b6bf1e1a0849d0866dccf4020e15b2e
