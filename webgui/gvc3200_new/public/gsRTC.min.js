...
function call(wsAddr, callback) {
    ...
    if( (gsRTC.getBrowserDetail().browser === 'chrome'  && gsRTC.getBrowserDetail().version < 72 ) ) {
        log.warn("当前版本不支持浏览器屏幕共享")
        callback(gsRTC.CODE_TYPE.NOT_SUPPORT_SCREEN_SHARE)
    }else {
...
GsRTC.prototype.shareScreen = function(data) {
    ...
    if(This.sharingPermission === 2 || This.sharingPermission === 3){
...
GsRTC.prototype.stopShareScreen = function(data) {
    ...
    if(gsRTC.sharingPermission === 2){
        This.sokect.sendMessage({type: gsRTC.SIGNAL_EVENT_TYPE.PRESENT_RET, ctrlPresentationRet: gsRTC.CODE_TYPE.SUCCESS, reqId: gsRTC.reqId})
    }else {
...
PeerConnection.prototype.setRemoteDescriptionSuccess = function () {
    ...
    }else if(gsRTC.sharingPermission === 0 || gsRTC.sharingPermission ===2 ) {
