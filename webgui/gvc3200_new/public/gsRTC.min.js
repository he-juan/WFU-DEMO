    ...
    MediaDevice.prototype.getMedia = async function (data, constraints) {
        ...
        function onGetStreamFailed(error) {
            if(gsRTC.sharingPermission == 3){
                clearInterval(gsRTC.timeBox)
                gsRTC.openSharingTimeoutstartTime = null
                gsRTC.openSharingTimestamp = null
            }
...
function call(wsAddr, callback) {
    ...
    if( !gsRTC.isWFUShareScreenSupport()) {
        ...
        callback(gsRTC.CODE_TYPE.NOT_SUPPORT_SCREEN_SHARE)
    }else {
        ...
        if(gsRTC.getBrowserDetail().browser === 'chrome'){
            function safariGetMediaCallBack(event){
                if(event.stream){
                    ...
                    callback(gsRTC.CODE_TYPE.CANCEL_PRESENT_ON)
                }
            }

            let data = {streamType: 'screenShare', callback: safariGetMediaCallBack}

            let getWebUIResolution = {}
            let webUIResolution
            gsRTC.getPreImgSize(function(event){
                webUIResolution = event
            })

            setTimeout(function(){
                getWebUIResolution = gsRTC.getResolutionAndFrame(webUIResolution)
                let constraints = {
                    audio: false,
                    video: {
                        width: {
                            ideal: getWebUIResolution.width,
                            max:  1920,
                        },
                        height: {
                            ideal:getWebUIResolution.height,
                            max: 1080,

                        },
                        frameRate: {
                            ideal: gsRTC.initialResolution && gsRTC.initialResolution.framerate || 15,
                            max: gsRTC.initialResolution && gsRTC.initialResolution.framerate || 15
                        }
                    }
                };
...
function beginScreen(callback){
    ...
    function getMediaCallBack(event){
        if(event.stream){
            if(gsRTC.openSharingTimestamp <= 30 ){
                try {
                    log.info('get stream success, ' + event.stream.id)
                    ...
                    gsRTC.RTCSession.setStream(stream, type, true)
                    data.stream = stream

                    if(gsRTC.sharingPermission === 3){
                        gsRTC.sharingPermission = 3
                    }else{
                ...
                gsRTC.openSharingTimestamp = null
                gsRTC.RTCSession.stopTrack(event.stream)
                gsRTC.sharingPermission = 1
            }
        }else {
            ...
            if(gsRTC.sharingPermission === 3){
                ...
                gsRTC.sharingPermission = 3
                gsRTC.sokect.sendMessage({type: gsRTC.SIGNAL_EVENT_TYPE.PRESENT_RET, ctrlPresentationRet:gsRTC.CODE_TYPE.CANCEL_PRESENT_ON, reqId: gsRTC.reqId})
                gsRTC.trigger( gsRTC.action,gsRTC.CODE_TYPE.CANCEL_PRESENT_ON)
            }
            ...
            gsRTC.CODE_TYPE.CANCEL_PRESENT_ON.rejectAuthorizationTip  = 'false'
        }
    }
    ...
    let constraints = {
        ...
        video: {
            width: {
                ideal: gsRTC.initialResolution && gsRTC.initialResolution.width || 1920,
                max:  gsRTC.initialResolution && gsRTC.initialResolution.width || 1920
            },
            height: {
                ideal: gsRTC.initialResolution && gsRTC.initialResolution.height ||1080,
                max: gsRTC.initialResolution && gsRTC.initialResolution.height || 1080
            },
            frameRate: {
                ideal: gsRTC.initialResolution && gsRTC.initialResolution.framerate || 15,
                max: gsRTC.initialResolution && gsRTC.initialResolution.framerate || 15
...
function stopScreen(callback){
    ...
    if(gsRTC.sharingPermission ===  2){
        gsRTC.sharingPermission =  2
    }else{
        gsRTC.sharingPermission = 0
    }
    if(gsRTC.openSharing === false){
        if(callback){
            callback(gsRTC.CODE_TYPE.PRESENT_ON_FAILED)
        }
    }else{
...
function hangUP(callback) {
    ...
    if(gsRTC.sharingPermission ===  4){
        gsRTC.sharingPermission =  4
    }else{
        gsRTC.sharingPermission = 5
    }
...
var GsRTC = function (options) {
    ...
    this.timeBox = null                                 //shareScreenRequest开启共享定时器
...
GsRTC.prototype.shareScreen = function(data) {
    ...
    log.info('current sharing permission: ' + This.sharingPermission)
    if(gsRTC.sharingPermission === 3){
        clearInterval(gsRTC.timeBox)
        gsRTC.openSharingTimeoutstartTime = null
        gsRTC.openSharingTimestamp = null
        This.sokect.sendMessage({type: This.SIGNAL_EVENT_TYPE.PRESENT_RET, ctrlPresentationRet: This.CODE_TYPE.SUCCESS, reqId: This.reqId})
        ...
        gsRTC.sharingPermission = 1
        This.sokect.sendMessage({type: This.SIGNAL_EVENT_TYPE.PRESENT, ctrlPresentation: { value: This.sharingPermission, reqId: This.reqId}})
    }
}
...
GsRTC.prototype.stopShareScreen = function(data) {
    ...
    log.info('current sharing permission: ' + gsRTC.sharingPermission)
    if(gsRTC.sharingPermission === 2){
        gsRTC.openSharing = false
        This.sokect.sendMessage({type: This.SIGNAL_EVENT_TYPE.PRESENT_RET, ctrlPresentationRet: This.CODE_TYPE.SUCCESS, reqId: This.reqId})
        This.trigger(This.action, This.CODE_TYPE.SUCCESS);
    }else if(gsRTC.sharingPermission === 0){
        gsRTC.openSharing = false
...
GsRTC.prototype.endCall = function(data){
    ...
    log.info('current sharing permission: ' + This.sharingPermission)
    if(this.sharingPermission == 4){
        This.sokect.sendMessage({type: gsRTC.SIGNAL_EVENT_TYPE.BYE_RET,destroyMediaSessionRet:This.CODE_TYPE.SUCCESS, reqId: This.reqId})
        This.trigger(This.action, This.CODE_TYPE.SUCCESS);
        gsRTC.cleanGsRTC()

    }else{
        This.sokect.sendMessage({type: gsRTC.SIGNAL_EVENT_TYPE.BYE,destroyMediaSession:{ value: This.sharingPermission }})
    }
}
...
GsRTC.prototype.cleanGsRTC = function(){
    ...
    This.sendCtrlPresentation = false
    This.closePeerConn()
...
PeerConnection.prototype.setRemoteDescriptionSuccess = function () {
    ...
    if(gsRTC.sharingPermission === 1 || gsRTC.sharingPermission === 3){
        gsRTC.openSharing = true
    }
...
PeerConnection.prototype.decorateLocalSDP = function () {
    ...
    sdp = This.gsRTC.deleteCodeByName(sdp)
        ...
        if(type=== 'slides' && !This.gsRTC.initialResolution){
            ...
            for(let fmtpItem of parseSDP.media[i].fmtp){
                if(fmtpItem.config.indexOf('profile-level-id') >= 0){
                    ...
                    log.warn("screen resolution = " ,This.gsRTC.initialResolution.width  + " * " + This.gsRTC.initialResolution.height)
...
GsRTC.prototype.CODE_TYPE = {
    ...
    INVALID_WEBSOCKET_ADDRESS: { codeType: 100, message: 'webSocket address is not a valid address' },

    CANCEL_PRESENT_ON:{codeType:403,message:'refuse to shareScreen',rejectAuthorizationTip:'false'},
    PRESENT_ON_REJECT: { codeType: 405, message: 'the call in Hold status!' },
    SHARE_SCREEN_REQUEST_REFUSE: { codeType: 201, message: 'Present turn On Request denied'  },
    STOP_SHARE_SCREEN_REQUEST_REFUSE: { codeType: 202, message: 'Present turn Off Request denied'  },
    HANG_UP_REQUEST_REFUSE:{codeType: 203, message: 'hang up Request denied' },

    NOT_SUPPORT_SCREEN_SHARE:{codeType:301,message:'The current browser version does not support Screen share'},
    SHARE_SCREEN_TIMEOUT:{codeType:408,message:'open shareScreen timeout'},
    REFUSE_CALL:{message:488,message:'Media information ERROR'}
    ...
    switch (This.serverAction) {
        case 'shareScreenRequest':
            if(confirm){
                ...
                This.sokect.sendMessage({type: This.SIGNAL_EVENT_TYPE.PRESENT_RET, ctrlPresentationRet: This.CODE_TYPE.SHARE_SCREEN_REQUEST_REFUSE, reqId: gsRTC.reqId})
                clearInterval(gsRTC.timeBox)
                gsRTC.openSharingTimeoutstartTime = null
                gsRTC.openSharingTimestamp = null
            }
        ...
        case 'stopShareScreenRequest':
            if(confirm){
                This.sharingPermission = 2
                stopScreen()
            }else {
                This.sokect.sendMessage({type: This.SIGNAL_EVENT_TYPE.PRESENT_RET, ctrlPresentationRet: This.CODE_TYPE.STOP_SHARE_SCREEN_REQUEST_REFUSE, reqId: gsRTC.reqId})
            }
            break
        case 'hangupRequest':
            if(confirm){
                ...
                This.sokect.sendMessage({type: This.SIGNAL_EVENT_TYPE.BYE_RET, destroyMediaSessionRet: This.CODE_TYPE.HANG_UP_REQUEST_REFUSE, reqId: gsRTC.reqId})
            }
            break
        default:
            break
...
/**
 * 从发送演示信令到取流的过程中计算时间，查看是否超时
 *
 */
GsRTC.prototype.openSharingTimeout = function(){
    log.info("Whether share desktop time out")
    gsRTC.timeBox = setInterval(function(){
        gsRTC.openSharingTimestamp = Math.round(((new Date()).getTime() - gsRTC.openSharingTimeoutstartTime)/1000)
        if(gsRTC.openSharingTimestamp > 30){
            log.info("shareScreen timeout")
            clearInterval(gsRTC.timeBox)
            gsRTC.openSharingTimeoutstartTime = null
            gsRTC.action = 'shareScreen'
            gsRTC.sokect.sendMessage({type: gsRTC.SIGNAL_EVENT_TYPE.PRESENT_RET, ctrlPresentationRet: gsRTC.CODE_TYPE.SHARE_SCREEN_TIMEOUT, reqId: gsRTC.reqId})
            gsRTC.trigger( gsRTC.action,gsRTC.CODE_TYPE.SHARE_SCREEN_TIMEOUT)

        }
    }, 1000)
}

GsRTC.prototype.getResolutionAndFrame =  function(getPreImgSize){
    log.info("get webUI Resolution and Frame")
    let resolution = {}
...
WebSocketInstance.prototype.createWebSocket = function(data){
    ...
    ws.onopen = function (event) {
        ...
        gsRTC.sokect.ws.websocketTimer = setInterval(function(){
            log.info("receive websocket keep alive!!!")
...
WebSocketInstance.prototype.handleIncomingMessage = function(message){
    ...
    if(action === 'ctrlPresentation'){
        gsRTC.openSharingTimeoutstartTime = (new Date()).getTime()
    }
    ...
    switch (action) {
        ...
        case gsRTC.SIGNAL_EVENT_TYPE.PRESENT_RET.name:
            if (gsRTC.isNxx(2, code)) {
                ...
                if (code === 403) {
                    gsRTC.trigger(gsRTC.action, {
                        codeType: gsRTC.CODE_TYPE.PRESENT_ON_REJECT,
                        message: data.rspInfo.rspMsg
                    });
                } else {
                    log.error(code + ', ' + data.rspInfo.rspMsg)
        ...
        case gsRTC.SIGNAL_EVENT_TYPE.BYE_RET.name:
            ...
            gsRTC.trigger(gsRTC.action, {
                codeType: code,
                message: gsRTC.CODE_TYPE.SUCCESS
            });
            break
        default:
            break
