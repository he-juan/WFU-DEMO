...
buildmenu:function($, setting){
	...
	$headers.each(function(i){
		...
		$curobj.children("li:eq(0)").css(this.istopheader? {paddingRight: smoothmenu.arrowimages.down[2]} : {}).append( //add arrow images
		...
		$curobj.hover(
			...
			function(e){
				...
				setTimeout(function(){$targetul.animate({height:'hide', opacity:'hide'}, ddsmoothmenu.transition.outtime)},500);
				if (smoothmenu.shadow.enabled){
...
init:function(setting){
	if (typeof setting.customtheme=="object" && setting.customtheme.length==2){
		...
		document.write('<style type="text/css">\n'
			+mainmenuid+', '+mainmenuid+' ul li {background:'+setting.customtheme[0]+';}\n'
			+mainmenuid+' ul li:hover {background:'+setting.customtheme[1]+';}\n'
		+'</style>')
	}
...
ddsmoothmenu.init({
	mainmenuid: "menuoption", //menu DIV id
	orientation: 'h',
	classname: 'ddsmoothmenu',
	//customtheme: ["#1c5a80", "#18374a"], //override default menu CSS background values? Uncomment: ["normal_background", "hover_background"]
	contentsource: ["menucontainer", "menu.htm"] //"markup" or ["container_id", "path_to_menu_file"]
})