...
tip_item.push (
	...
	new TipsDef("Use H.264 Constrained Profiles", "使用H.264 Constrained Profiles",
        "设置是否使用H.264 CBP，从而与WebRTC视频进行正常建立。此项仅当H.264 Profile类型设置中包含BP类型（基本档次）时有效。与WebRTC建立视频通话时建议开启。默认为否。"),
	new TipsDef("H.265 Payload Type", "H.265有效荷载类型",
	    "输入H.265有效荷载值，范围为：96-127,默认为114。"),
	...
	new TipsDef("Enable RTX", "丢包重传",
	    "当开启该功能时，信令会携带RTX信息，若最终协商成功，会使能媒体RTX相关功能实现丢包重传的目的。当关闭该功能时，则无法应用丢包重传。"),
	...
	new TipsDef("Enable DND Forword", "开启勿扰转移",
    ...
    new TipsDef("Personal Common Layout", "常用布局模式",
        "设置会议常用布局模式，设置的模式将应用所有会议，默认系统推荐布局模式。"),
    ...
    new TipsDef("DHCP VLAN Override", "DHCP VLAN模式",
        "选择DHCP Option VLAN模式。若选择“DHCP选项132和DHCP选项133”，设备将接收DHCP服务器发来的DHCP option 132 (802.1Q VLAN ID) 以及DHCP option 133 (QOS priority level)用于本地网络设置。若选择“封装于DHCP选项43”，设备将接收DHCP服务器发来的DHCP option 43用于本地网络设置。注：当选择“封装于DHCP Option 43”时，请确保维护->更新升级中的“启动DHCP选项43、160和66服务器设置”设置项已启用。默认为“禁用”。"),
 ...
 /*Maintenance Web Access*/
    ...
    new TipsDef("Auto Reboot to Upgrade Without Prompt", "升级时不弹出确认框",
		"若选择“是”，在下载完升级固件后自动升级。否则在升级前弹出确认提示。默认为“否”。"),
	...
	new TipsDef("One-click Debugging", "一键调试",
		"抓取调试清单中勾选的信息，包含“抓包”项时需点击“开始”进行调试，点击“停止”结束，其余情况点击“抓取”即可。所有获取的文件将会生成压缩包，上一次生成的压缩包会被覆盖，抓包文件则会一直保留。"),
	new TipsDef("Debug Information List", "调试信息清单",
		"展示可供抓取调试信息项列表，目前支持系统日志，信息日志和抓包。抓取的数据可在“调试信息列表”里查看。默认全部选中。"),
    new TipsDef("Package List", "已有调试信息列表",
		"可选择已有的调试信息压缩包或抓包文件。点击右方的“删除”按钮可删除该文件。"),
	new TipsDef("View Package", "查看已有调试信息",
   ...
   new TipsDef("Flicker Control", "闪动控制",
    ...
    new TipsDef("Move Speed", "移动速度",
        "设置镜头转动和缩放速度。共16档，1档最慢，16档最快。默认为1档。"),
    ...
    new TipsDef("Visual Angle", "可视角度",
		"默认值（小）时是当前正常视角，当调整到大时需对应不同的畸变矫正参数，本地端摄像头视角随之调整。"),
	...
	new TipsDef("Enable Presentation Automatically When HDMI Plugged", "当接入HDMI时自动开启演示",
		"选择“是”则接入HDMI时自动显示演示。选择“否”则接入HDMI时会弹框提示用户选择确认或者取消，如果用户不点击确认或者取消，则超时30s就自动显示演示。默认为“否”。"),
    ...
    new TipsDef("Audio Device", "音频设备",
        "该项是选择通话/媒体声音的输入输出设备，分为自动、蓝牙、USB、HDMI、内置扬声器和鹅颈麦。选择“USB”，声音从USB设备输入和输出。选择“蓝牙”，声音从蓝牙设备输入和输出。选择“HDMI”，声音从GVC内置Mic输入，从HDMI 设备输出。选择“内置扬声器”，声音从GVC内置Mic输入，从内置扬声器输出。选择“鹅颈麦”，并且输出设备为HDMI时, 声音从USB设备输入，从HDMI设备输出。选择“自动”，会自动检测是否有连接USB、蓝牙、HDMI 、内置扬声器，当同时有连接其中两个或以上，声音输入和输出优先级为蓝牙>USB>HDMI>内置扬声器。默认为自动。"),
